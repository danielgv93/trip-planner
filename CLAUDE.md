...
explicitly with an existing GitHub JSON file.
...
- Preserve the established state mutation cycle: `save(); render(); drawMap();`.
  `render()` destructively rebuilds the itinerary DOM.
...
- Current feature domains include planner, map, finance, notes, companion,
  health, GitHub synchronization, the LLM assistant, and workspace resizing.

import { store, save, dayBy, routeTimeProfile, routeTimeOverride } from "../../core/store.js?v=26";
import { $, esc } from "../../shared/dom.js";
import { toast } from "../../shared/notify.js?v=3";
import { buildTimelineProjection } from "../companion/timeline.js";
import { cachedRouteTravelMinutes, ensureRouteTravelTimes, drawMap } from "../map/map.js";
...
function travelForLeg(from, to) {
    ...
    return { minutes: override ?? officialMinutes, officialMinutes, overridden: override !== null, profile };
}
...
async function hydrateDay(day) {
    ...
    await Promise.all([...profiles].map((profile) => ensureRouteTravelTimes(spots, profile)));
}
...
function renderCenter(focusDayId) {
    ...
    const overviewStates = ["solid", "tight", "impossible", "unchecked"];
    ...
    resultsEl.innerHTML = store.state.map((day, dayIndex) => {
        ...
        const suggestions = result.state !== "unchecked" ? generateSuggestions(day, result, evaluate, store.state) : [];
...
export async function checkPlan({ focusDayId } = {}) {
    ...
    for (let index = 0; index < store.state.length; index += 1) {
        if (token !== runToken) return;
        const day = store.state[index];
        $("#healthProgress").textContent = `Comprobando ${index + 1} de ${store.state.length}: ${day.title || day.date}…`;
        try { await hydrateDay(day); } catch { /* timeline will use geographic estimates */ }
        if (token !== runToken) return;
        setHealthResult(day, evaluate(day));
    }
    busy = false;
...
resultsEl.addEventListener("click", async (event) => {
    ...
    if (action === "edit") {
        ...
        if (spot) { dialog.close(); openDialog(day.id, spot, { focus: button.dataset.focus }); }
        return;
    }
